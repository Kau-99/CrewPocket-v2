...
## ADR-030 — Revisão completa: 6 correções; flake de listen do emulator mitigado com retry

**Data:** 2026-06-12 · **Fase:** pós-deploy (revisão)
...
## ADR-033 — Estimates muito mais ricos: line items, termos+depósito, templates, assinatura

**Data:** 2026-06-13 · **Fase:** pós-deploy (a pedido do Kauã)
Contexto: pedido de deixar o app "igual a v1, com bastante funcionalidade", focado em job e estimate. Job já enriquecido (ADR sem nº, commit 41e2d85). Para o estimate, 4 frentes escolhidas pelo Kauã.
Decisão: (1) line items ganham `unit` e `note` por linha + duplicar (editor compartilhado; campos adicionados também ao invoiceLineItem p/ consistência); (2) `terms`, `depositType` (none/fixed/percent) e `depositValue` no estimate, com depósito devido derivado e exibido no detalhe e no PDF; (3) coleção `estimateTemplates` (rules owner-only na matriz, 51 rules) com "salvar como template" no detalhe e "novo a partir de template" na lista; (4) assinatura do cliente capturada em `<canvas>` e gravada como data URL no próprio doc (sem Storage ainda; cap 400KB nas rules) + nome + data, anexada ao PDF. Todos os campos novos são opcionais/default → docs já gravados continuam válidos.
Consequência: estimate vira documento comercial completo. Data URL da assinatura no doc é pragmático enquanto o Storage (Blaze) não está ligado; migrar p/ Storage depois se crescer. PDF do estimate agora mostra unit/nota/termos/depósito/assinatura; invoice ignora (props opcionais).

## ADR-034 — PWA instalável de primeira classe (Android + iOS)

**Data:** 2026-06-13 · **Fase:** pós-deploy (a pedido do Kauã: "baixável em qualquer celular")
Contexto: o app já era PWA, mas faltava a experiência de instalação polida — iOS não tinha as metatags Apple (instala "torto", sem nome/ícone/standalone) e não havia convite de instalação em lugar nenhum.
Decisão: (1) metadata `appleWebApp` (capable + título + status bar translúcida) + `apple-touch-icon` + `viewport-fit=cover` (atrás do notch) + `formatDetection.telephone=false`; (2) manifest enriquecido (id, scope, display_override, orientation, categories, lang/dir, shortcuts p/ Clock in / New job / New estimate); (3) componente InstallPrompt — Android/desktop usam o prompt nativo (beforeinstallprompt) com botão; iOS Safari (sem prompt) mostra instruções "Compartilhar → Add to Home Screen"; dispensável via localStorage, some quando standalone; (4) shortcuts ?new=1 abrem o form direto (jobs/estimates) — useSearchParams OK porque tudo é force-dynamic (ADR-029).
Consequência: instalável e profissional nos dois SOs sem app store. Splash screens do iOS ficaram de fora (exigiriam ~20 PNGs por device) — aceitável p/ v1.

## ADR-035 — Reset de senha via Resend (entrega confiável) com fallback Firebase

**Data:** 2026-06-14 · **Fase:** pós-deploy (a pedido do Kauã)
Contexto: o e-mail de reset do Firebase (noreply@<proj>.firebaseapp.com) cai no spam do Gmail — o app pedia o reset certo (API 200) mas o usuário nunca recebia. Entrega não-confiável é inaceitável p/ produto.
Decisão: rota `/api/auth/reset-password` gera o link com Admin `generatePasswordResetLink` e envia um e-mail HTML branded via Resend (REST direto, sem SDK). Anti-enumeração (user-not-found → 200), rate limit. `lib/auth/reset.requestPasswordResetEmail` tenta a rota; 501 (sem Resend) ou erro → fallback p/ o `sendPasswordResetEmail` do Firebase. Login-form e account-section usam o mesmo helper. RESEND*API_KEY/RESEND_FROM opcionais (build não quebra).
Consequência: funciona antes (fallback Firebase) e depois (Resend) de configurar, degradação graciosa. Requer, p/ o caminho Resend: RESEND*\* + service account real do Firebase na Vercel (link gerado pelo Admin). Resend grátis envia de onboarding@resend.dev só p/ o dono; produção precisa domínio verificado.
